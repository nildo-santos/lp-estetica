...
.service-page { flex: 0 0 100%; min-width: 0; display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); align-items: stretch; grid-template-rows: auto auto 1fr; gap: 0 12px; padding: 4px; scroll-snap-align: start; scroll-snap-stop: always; }
.service-card { display: grid; grid-template-rows: subgrid; grid-row: span 3; min-width: 0; overflow: hidden; border: 1px solid #eae5dc; border-radius: var(--radius-card); background: var(--color-surface); text-align: left; box-shadow: 0 2px 4px #0000000d; }
.service-card:hover { border-color: #d4af37; }
.service-card img { display: block; width: 100%; aspect-ratio: 16 / 10; object-fit: cover; }
.service-card > h4 { display: flex; align-items: center; min-height: calc(4.5em + 32px); margin: 0; padding: 16px; font-size: 14px; line-height: 1.5; font-weight: 500; color: #3f3f46; overflow-wrap: anywhere; }
...
.service-details { align-self: start; margin: 0 12px 12px; border-top: 1px solid #eae5dc; }
