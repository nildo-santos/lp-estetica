...
<template>
  <nav :class="['contact-links', { floating }]" :aria-label="floating ? 'Atalhos flutuantes de contato' : 'Canais de contato'">
    <a v-for="link in contactLinks" :key="link.id" :href="link.href" :target="link.external ? '_blank' : undefined" :rel="link.external ? 'noopener noreferrer' : undefined" :class="['contact-button', link.id]" :aria-label="link.label" :title="link.label">
      ...
      <span v-if="!floating" class="contact-copy"><strong>{{ link.label }}</strong><span v-if="link.id === 'email'">{{ link.detail }}</span></span>
...
.floating { position: fixed; right: max(18px, env(safe-area-inset-right)); bottom: max(20px, env(safe-area-inset-bottom)); z-index: 40; display: flex; flex-direction: column; gap: 10px; }
.floating .contact-button { position: relative; padding: 0; width: 52px; height: 52px; border: 2px solid #fff; border-radius: 50%; box-shadow: 0 4px 16px #392b1133; }
.floating .contact-icon { width: 100%; height: 100%; }
.contact-tooltip { position: absolute; right: calc(100% + 12px); padding: 6px 12px; border-radius: 8px; background: #3f3931; color: #fff; font-size: 12px; opacity: 0; pointer-events: none; transition: opacity .2s; }
.contact-button:hover .contact-tooltip, .contact-button:focus-visible .contact-tooltip { opacity: 1; }
@media (max-width: 767px) { .contact-links:not(.floating) { grid-template-columns: 1fr; } .floating { right: max(12px, env(safe-area-inset-right)); bottom: max(12px, env(safe-area-inset-bottom)); gap: 8px; } .floating .contact-button { width: 46px; height: 46px; } }
@media (prefers-reduced-motion: reduce) { .contact-button, .contact-tooltip { transition: none; } .contact-button:hover { transform: none; } }

@media (max-width: 767px) {
  .floating { flex-direction: row; padding: 8px; border: 1px solid #d4af3733; border-radius: 999px; background: #fdfbf7f2; box-shadow: 0 4px 20px #392b1120; }
  .floating .contact-tooltip { right: 50%; bottom: calc(100% + 12px); transform: translateX(50%); }
}
</style>
